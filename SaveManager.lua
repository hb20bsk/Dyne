...
local SaveManager = {} do
    ...
    SaveManager.Parser = {
        ...
        KeyPicker = {
            Save = function(idx, object)
                return { type = 'KeyPicker', idx = idx, mode = object.Mode, key = object.Value and object.Value.Name or "None", modifiers = object.Modifiers }
            end,
            Load = function(idx, data)
                if SaveManager.Library.Options[idx] then
                    local keyCode = data.key ~= "None" and Enum.KeyCode[data.key] or nil
                    SaveManager.Library.Options[idx]:SetValue({ keyCode, data.mode, data.modifiers })
                end
            end,
        },
        Input = {
    ...
    function SaveManager:LoadAutoloadConfig()
        ...
        if isfile(autoLoadPath) then
            ...
            if not successRead then
                self.Library:Notify({
                    Title = "Error",
                    Description = 'Failed to load autoload config: read file error',
                    Duration = 3,
                    Type = "error"
                })
                return
    ...
    function SaveManager:BuildConfigSection(tab)
        ...
        section:AddButton({
            ...
            Func = function()
                ...
                self.AutoloadLabel:SetText('Current autoload: ' .. name)
            end
        })
        ...
        section:AddButton({
            ...
            Func = function()
                ...
                self.Library:Notify({
                    Title = "Success",
                    Description = 'Reset autoload to none',
                    Duration = 3,
                    Type = "success"
                })
        ...
        self.AutoloadLabel = section:AddLabel("Current autoload: " .. self:GetAutoloadConfig())
